...
if ($resp==1) {
 ...
 foreach($result as $row){
  ...
  $tmp=explode("_",$json_data["RunID"]);
  $RunID=$tmp[0];
  if (is_file($elfinderFolder_USER."/".$id."/".$RunID."_check.png") && is_file($elfinderFolder_USER."/".$id."/".$RunID.".png")){
   $identify_file_01=$elfinderFolder_USER."/".$id."/".$RunID."_check.png";
   $identify_file_02=$elfinderFolder_USER."/".$id."/".$RunID.".png";  
  }
  $srrNum=basename($json_data["srrNum"],"_R2.fastq.gz");
  $record_file=$elfinderFolder_USER."/".$id."/".$srrNum.".record";
